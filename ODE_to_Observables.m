...
for t=1:length(T)  
    nHospital(T(t)+LL,A)=nHospital(T(t)+LL,A) + Distribution_Symptoms_to_Hospital'*(nDC(T(t),A).*Prob_Modifier(Sympt_2_hosp(A),h_factor)');
end
...
for t=1:length(T)
    nICU(T(t)+LL,A)=nICU(T(t)+LL,A) + Distribution_Symptoms_to_ICU' * (nDC(T(t),A).*Prob_Modifier(Sympt_2_critcal(A),i_factor)');
end
...
for t=1:length(T)
    nDeaths(T(t)+LL+Assumed_Delay_Reporting_Deaths(Region),A)=nDeaths(T(t)+LL+Assumed_Delay_Reporting_Deaths(Region),A) + Distribution_Hopital_to_Death'*(nHospital(T(t),A).*Prob_Modifier(Hosp_2_Death(A),d_factor)');
end
nDeaths((max(T)+1):end,:)=[];
nDC((max(T)+1):end,:)=[];
